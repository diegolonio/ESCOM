...
/* LEDs en PORTD */
#define LED_VERDE PD5 // pasiv0
#define LED_AMARILLO PD6 // golpe
#define LED_AZUL PD7 // caida

/* Indices de clase:  0 = caida, 1 = golpe, 2 = pasivo */
...
int main(void)
{
    ...
    while (1) {
        ...
        /* Clasificar en cada impacto. Caida tiene prioridad. */
        if (m > (float)TH_PICO) {
            uint8_t clase = clasificar(buf);

            if (clase == CLASE_CAIDA) {
                hold = HOLD_TICKS;
                hold_clase = CLASE_CAIDA; // caida manda siempre
